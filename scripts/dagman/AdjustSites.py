...
It not only does adjusting sites (blacklist/whitelist) but also:
    ...
    - For resubmission: handled by DAGMan rescue with retries reset; no local file surgery
"""
...
def getGlob(ad, normal, automatic):
    """Return list of files for automatic splitting vs conventional DAG."""
...
def main():
    ...
    # No more direct edits of .nodes.log or DAG files on resubmission.
    # DAGMan will reset retries upon rescue; we only log intent here.
    if 'CRAB_ResetRetries' in ad and ad['CRAB_ResetRetries']:
        printLog("Resubmission detected (CRAB_ResetRetries=True). Relying on DAGMan rescue mode to reset retries.")
    else:
        printLog("No retry-reset requested. Proceeding normally.")
