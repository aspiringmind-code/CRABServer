...
def writeDagmanConfigFile(dagFilename, maxRetries):
    """
    Create a DAGMan configuration file to enable automatic retry resets via Rescue DAGs.
    dagFilename: path to main DAG (e.g. RunJobs.dag)
    maxRetries: number of automatic retries per node
    """
    cfgFile = os.path.splitext(dagFilename)[0] + ".dag.config"
    with open(cfgFile, "w", encoding="utf-8") as cfg:
        ...
        cfg.write(f"DAGMAN_DEFAULT_RETRY            = {maxRetries}\n")
    return cfgFile
...
class DagmanCreator(TaskAction):
    ...
    def createSubdag(self, splitterResult, **kwargs):
        ...
        # Create the DAGMan config to enable automatic retry resets via Rescue DAGs
        cfgPath = writeDagmanConfigFile("RunJobs.dag", self.task['numautomjobretries'])
        self.logger.info(f"Created DAGMan config file: {cfgPath}")
